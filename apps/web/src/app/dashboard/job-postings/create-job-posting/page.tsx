...
const CreateJobPosting = () => {
  ...
  const handleChange = (
    ...
    const formattedValue =
      name === 'salary' ? formatInputWithCommas(value) : value;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append('admin_id', user.id.toString()); // Replace with the actual admin ID
    ...
    if (jobData.salary) {
      formData.append(
        'salary',
        parseFloat(jobData.salary.replace(/,/g, '')).toString(),
    ...
    if (response.ok) {
      router.push(
        '/dashboard/job-postings/create-job-posting/confirmation?status=posted',
      );
    } else {
    ...
    <DashboardLayout>
      <div className="flex flex-col p-4 bg-gray-500">
        ...
        <form
          ...
          className="w-full mx-auto bg-white p-6 rounded-lg shadow-md border border-gray-300"
        >
          <div className="mb-4">
